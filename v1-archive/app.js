...
const setBuilderSection = document.getElementById("setBuilder");
...
const openSetBuilderFromSetsButton = document.getElementById("openSetBuilderFromSets");
...
const favoriteRecipesTab = document.getElementById("favoriteRecipesTab");
const favoriteSetsTab = document.getElementById("favoriteSetsTab");
const favoriteRecipesPanel = document.getElementById("favoriteRecipesPanel");
const favoriteSetsPanel = document.getElementById("favoriteSetsPanel");
const favoriteSetSearch = document.getElementById("favoriteSetSearch");
const favoriteSetList = document.getElementById("favoriteSetList");
const favoriteSetEmpty = document.getElementById("favoriteSetEmpty");
...
let pendingCompleteRecipe = null;
let pendingCompleteSet = null;
...
function createDishEntry({ recipeId = null, servings = 0, draftName = "", cooked = false } = {}) {
  const entry = {
    id: createId("dish"),
...
let favoriteView = "recipes";

function setFavoriteView(view) {
  favoriteView = view;
  if (favoriteRecipesPanel) {
    favoriteRecipesPanel.hidden = view !== "recipes";
  }
  if (favoriteSetsPanel) {
    favoriteSetsPanel.hidden = view !== "sets";
  }
  if (favoriteRecipesTab) {
    favoriteRecipesTab.classList.toggle("favorites-tabs__button--active", view === "recipes");
    favoriteRecipesTab.setAttribute("aria-selected", view === "recipes" ? "true" : "false");
  }
  if (favoriteSetsTab) {
    favoriteSetsTab.classList.toggle("favorites-tabs__button--active", view === "sets");
    favoriteSetsTab.setAttribute("aria-selected", view === "sets" ? "true" : "false");
  }
  if (view === "sets") {
...
function renderFavoriteSetsList() {
  if (!favoriteSetList || !favoriteSetEmpty) return;
  const query = favoriteSetSearch ? favoriteSetSearch.value.trim().toLowerCase() : "";
  const favorites = getAllSets().filter(set => isSetFavorite(set.id));
  const filtered = query
    ? favorites.filter(set => {
  ...
  if (filtered.length === 0) {
    favoriteSetEmpty.hidden = false;
    return;
  }
  favoriteSetEmpty.hidden = true;
  filtered.forEach(set => renderSetCard(set, favoriteSetList));
}

function resetSetBuilder({ preselect = [] } = {}) {
  setBuilderSelection = Array.from(new Set(preselect));
  if (setBuilderName) setBuilderName.value = "";
  if (setBuilderDescription) setBuilderDescription.value = "";
  if (setBuilderTags) setBuilderTags.value = "";
  if (setBuilderSearch) setBuilderSearch.value = "";
  renderSetBuilderSelected();
  renderSetBuilderList();
}

function renderSetBuilderSelected() {
  if (!setBuilderSelected || !setBuilderSelectedEmpty) {
    return;
  }
  setBuilderSelected.innerHTML = "";
  if (setBuilderSelection.length === 0) {
    setBuilderSelectedEmpty.hidden = false;
    return;
  }
  setBuilderSelectedEmpty.hidden = true;
  setBuilderSelection.forEach((id) => {
    const recipe = getRecipeById(id);
    if (!recipe) return;
    const chip = document.createElement("button");
    chip.type = "button";
    chip.className = "set-builder__chip";
    chip.innerHTML = `<span>${recipe.name}</span><span class="set-builder__chip-close">×</span>`;
    chip.addEventListener("click", () => {
      setBuilderSelection = setBuilderSelection.filter(item => item !== id);
      renderSetBuilderSelected();
      renderSetBuilderList();
    });
    setBuilderSelected.appendChild(chip);
  });
}

function renderSetBuilderList() {
  if (!setBuilderList || !setBuilderEmpty) {
    return;
  }
  if (recipeDb.length === 0) {
    setBuilderList.innerHTML = "";
    setBuilderEmpty.hidden = false;
    return;
  }
  setBuilderEmpty.hidden = true;
  const query = setBuilderSearch ? setBuilderSearch.value.trim().toLowerCase() : "";
  const list = query
    ? recipeDb.filter(recipe => {
        const name = recipe.name.toLowerCase();
        const tags = (recipe.tags || []).join(" ").toLowerCase();
        const ingredients = recipe.ingredients.map(ing => ing.name).join(" ").toLowerCase();
        return name.includes(query) || tags.includes(query) || ingredients.includes(query);
      })
    : recipeDb;
  ...
  if (list.length === 0) {
    ...
    empty.className = "set-builder__empty";
    empty.textContent = "一致するレシピがありません";
    setBuilderList.appendChild(empty);
    return;
  }

  list.forEach((recipe) => {
    const item = document.createElement("article");
    item.className = "set-builder__card";
    const isSelected = setBuilderSelection.includes(recipe.id);
    const ingredients = Array.isArray(recipe.ingredients) ? recipe.ingredients : [];
    item.innerHTML = `
      <div class="set-builder__card-main">
        <h3>${recipe.name}</h3>
        <p>${ingredients.slice(0, 3).map(ing => ing.name).join("、")}${ingredients.length > 3 ? " 他" : ""}</p>
      </div>
      <button type="button" class="button--small ${isSelected ? "button--primary" : "button--ghost"}">
        ${isSelected ? "選択中" : "追加"}
      </button>
    `;
    ...
    button.addEventListener("click", () => {
      if (isSelected) {
        setBuilderSelection = setBuilderSelection.filter(itemId => itemId !== recipe.id);
      } else {
        setBuilderSelection = [...setBuilderSelection, recipe.id];
      }
      renderSetBuilderSelected();
      renderSetBuilderList();
    });
    setBuilderList.appendChild(item);
  });
}

function collectSetBuilderTags() {
  if (!setBuilderTags) return [];
  return setBuilderTags.value
    .split(/[,\u3001]/)
    .map(tag => tag.trim())
    .filter(Boolean);
}

function openRecipeCompleteModal(recipe) {
  if (!recipeCompleteModal || !recipeCompleteMessage) return;
  pendingCompleteRecipe = recipe;
  recipeCompleteMessage.textContent = `「${recipe.name}」を登録しました。`;
  recipeCompleteModal.hidden = false;
  document.body.style.overflow = "hidden";
}

function closeRecipeCompleteModal() {
  if (!recipeCompleteModal) return;
  recipeCompleteModal.hidden = true;
  document.body.style.overflow = "";
  pendingCompleteRecipe = null;
}

function openSetCompleteModal(set) {
  if (!setCompleteModal || !setCompleteMessage) return;
  pendingCompleteSet = set;
  setCompleteMessage.textContent = `「${set.name}」を作成しました。`;
  setCompleteModal.hidden = false;
  document.body.style.overflow = "hidden";
}

function closeSetCompleteModal() {
  if (!setCompleteModal) return;
  setCompleteModal.hidden = true;
  document.body.style.overflow = "";
  pendingCompleteSet = null;
}

function confirmSetBuilderAction() {
  if (!setBuilderName) return;
  const name = setBuilderName.value.trim();
  if (!name) {
    setBuilderName.focus();
    return;
  }
  if (setBuilderSelection.length === 0) {
    showToast("レシピを選んでください", "info");
    return;
  }

  const description = setBuilderDescription ? setBuilderDescription.value.trim() : "";
  const tags = collectSetBuilderTags();
  const dishes = setBuilderSelection.map((id) => {
    const recipe = getRecipeById(id);
    return {
...
function openRecipesView({ scroll } = { scroll: true }) {
  ...
  if (setBuilderSection) {
    setBuilderSection.hidden = true;
  }
  updateNavigation("recipes");
  setFavoriteView(favoriteView || "recipes");
  renderTagFilter();
  renderRecipeList();
  if (scroll) {
...
function openPublicRecipesView({ scroll } = { scroll: true }) {
  ...
  updateNavigation("publicRecipes");
  renderPublicRecipes();
...
function syncViewFromHash() {
  ...
  if (window.location.hash === "#weeklySets") {
    openWeeklySetsView({ scroll: false });
    return;
  }
  if (window.location.hash === "#setBuilder") {
    openSetBuilderView({ scroll: false });
    return;
  }
...
function renderUnifiedDishCard({ dish, recipe, dayKey }) {
  ...
  if (dish.cooked) {
    card.classList.add("unified-dish-card--cooked");
  }
  ...
  titleWrap.className = "unified-dish-card__title";
  ...
  if (dish.cooked) {
    ...
    cookedBadge.className = "unified-dish-card__badge";
    cookedBadge.textContent = "作った";
    titleWrap.appendChild(cookedBadge);
  }

  compact.appendChild(titleWrap);
  ...
  cookedButton.className = `button--small ${dish.cooked ? "button--done" : "button--ghost"}`;
  cookedButton.textContent = dish.cooked ? "作った！" : "作った";
  ...
  clearButton.textContent = "リストから外す";
  ...
  actions.appendChild(cookedButton);
...
function getPoolDayKey(weekStart = currentWeekStart) {
  return formatDate(weekStart);
}

function getWeekDishEntries(weekStart) {
  const entries = [];
  for (let index = 0; index < 7; index += 1) {
    ...
    dayData.dishes.forEach((dish) => {
      if (!dish.recipeId && !dish.draftName) {
        return;
      }
      entries.push({ dayKey: dateKey, dish });
    });
  }
  return entries;
}

function addDishToPool({ recipe, draftName = "" }) {
  const dayKey = getPoolDayKey();
  const entry = getDayData(dayKey);
  entry.dishes = entry.dishes.filter(dish => dish.recipeId || dish.draftName);
  const payload = recipe
    ? {
        recipeId: recipe.id,
        servings: normalizeBaseServings(recipe.baseServings),
      }
    : { draftName };
  const dish = createDishEntry(payload);
  entry.dishes.push(dish);
  commitSave();
  renderWeek(currentWeekStart);
  return { dayKey, dishId: dish.id };
}

function renderPoolAddResults({ query, container, input }) {
  container.innerHTML = "";
  const trimmed = query.trim();
  const matches = trimmed ? searchRecipes(trimmed).slice(0, 6) : recipeDb.slice(0, 6);
  const exactMatch = trimmed ? getRecipeByName(trimmed) : null;
  ...
  if (trimmed && !exactMatch) {
    ...
    createButton.type = "button";
    createButton.className = "recipe-search__item recipe-search__item--create";
    createButton.textContent = `「${trimmed}」をレシピとして登録`;
    createButton.addEventListener("click", () => {
      const { dayKey, dishId } = addDishToPool({ draftName: "" });
      openRecipeModal({ dayKey, dishId, name: trimmed });
      input.value = "";
      container.hidden = true;
    });
...
function renderDraftDishCard({ dish, dayKey }) {
  const card = document.createElement("div");
  card.className = "pool-draft";

  const title = document.createElement("p");
  title.className = "pool-draft__title";
  title.textContent = dish.draftName || "名前未設定";
  card.appendChild(title);

  const actions = document.createElement("div");
  actions.className = "pool-draft__actions";

  const registerButton = document.createElement("button");
  registerButton.type = "button";
  registerButton.className = "button--primary button--small";
  registerButton.textContent = "レシピに登録";
  registerButton.addEventListener("click", () => {
    openRecipeModal({ dayKey, dishId: dish.id, name: dish.draftName });
  });

  const removeButton = document.createElement("button");
  removeButton.type = "button";
  removeButton.className = "button--ghost button--small button--danger";
  removeButton.textContent = "削除";
  removeButton.addEventListener("click", () => {
    removeDish(dayKey, dish.id);
  });
...
function getPoolStats(weekStart) {
  let total = 0;
  let cooked = 0;
  for (let index = 0; index < 7; index += 1) {
    const date = new Date(weekStart);
...
function updatePoolStatus(weekStart) {
  if (!poolProgressBar || !poolCount) {
    return;
  }
  const stats = getPoolStats(weekStart);
  const remaining = Math.max(stats.total - stats.cooked, 0);
  const progress = stats.total > 0 ? stats.cooked / stats.total : 0;
  poolCount.textContent = `${remaining} / ${stats.total}`;
  poolProgressBar.style.width = `${Math.round(progress * 100)}%`;
  poolProgressBar.setAttribute("aria-valuenow", String(Math.round(progress * 100)));
}
...
function renderWeek(weekStart) {
  ...
  const appliedSet = loadAppliedSet(weekStart);
  summary.innerHTML = `
    <p class="eyebrow">Set Flow</p>
    <h2>${appliedSet ? appliedSet.setName : "今週のセット料理"}</h2>
    <p class="pool-summary__lead">${
      appliedSet
        ? "順番は自由。何から作ってもOK。"
        : "セットを選ぶか、レシピを追加して始めよう。"
    }</p>
  `;
  weekGrid.appendChild(summary);

  const addSection = document.createElement("div");
  addSection.className = "pool-add";

  const addLabel = document.createElement("label");
  addLabel.className = "pool-add__label";
  addLabel.textContent = "レシピを追加";

  const addInput = document.createElement("input");
  addInput.type = "search";
  addInput.placeholder = "料理名・食材・タグで検索";
  addInput.className = "pool-add__input";

  const addResults = document.createElement("div");
  addResults.className = "recipe-search__results pool-add__results";
  addResults.hidden = true;

  addInput.addEventListener("input", () => {
    renderPoolAddResults({ query: addInput.value, container: addResults, input: addInput });
  });
  ...
  addInput.addEventListener("keydown", (event) => {
    ...
    if (existing) {
      addDishToPool({ recipe: existing });
    } else {
      addDishToPool({ draftName: trimmed });
    }
    addInput.value = "";
    addResults.hidden = true;
  });
  ...
  weekGrid.appendChild(addSection);

  const poolItems = document.createElement("div");
  poolItems.className = "pool-items";

  const entries = getWeekDishEntries(weekStart);
  if (entries.length === 0) {
    const empty = document.createElement("p");
    empty.className = "pool-empty";
    empty.textContent = "セットを選ぶか、レシピを追加して始めよう";
    poolItems.appendChild(empty);
  } else {
    entries.forEach(({ dayKey, dish }) => {
      const selectedRecipe = dish.recipeId ? getRecipeById(dish.recipeId) : null;
      const item = document.createElement("article");
      item.className = "pool-item";
      if (dish.cooked) {
        item.classList.add("pool-item--cooked");
      }

      if (selectedRecipe) {
        item.appendChild(renderUnifiedDishCard({ dish, recipe: selectedRecipe, dayKey }));
        item.appendChild(renderServingControl({ dish, recipe: selectedRecipe }));
        ...
        item.appendChild(deletedNotice);
      } else if (dish.draftName) {
        item.appendChild(renderDraftDishCard({ dish, dayKey }));
      }
  ...
  weekGrid.appendChild(poolItems);

  renderShoppingList();
  setSaveStatus("保存済み");
  updatePoolStatus(currentWeekStart);
}
...
window.addEventListener("keydown", (event) => {
  ...
  if (recipeCompleteModal && !recipeCompleteModal.hidden) {
    closeRecipeCompleteModal();
  }
  if (setCompleteModal && !setCompleteModal.hidden) {
    closeSetCompleteModal();
  }
});
...
recipeForm.addEventListener("submit", (event) => {
  ...
  renderTagFilter();
  renderWeek(currentWeekStart);
  showToast(wasEditing ? "レシピを更新しました" : "レシピを登録しました", "success");

  if (!wasEditing && createdRecipe) {
    openRecipeCompleteModal(createdRecipe);
  }
});
...
const openWeeklySetsFromPublicButton = document.getElementById("openWeeklySetsFromPublic");
const openPublicRecipesFromSetsButton = document.getElementById("openPublicRecipesFromSets");

const exploreTabSets = document.getElementById("exploreTabSets");
const exploreTabRecipes = document.getElementById("exploreTabRecipes");
const exploreTabSetsFromPublic = document.getElementById("exploreTabSetsFromPublic");
const exploreTabRecipesFromPublic = document.getElementById("exploreTabRecipesFromPublic");
...
if (openWeeklySetsFromPublicButton) {
  openWeeklySetsFromPublicButton.addEventListener("click", () => {
    openWeeklySetsView({ scroll: true });
    history.replaceState(null, "", "#weeklySets");
  });
}

if (openPublicRecipesFromSetsButton) {
  openPublicRecipesFromSetsButton.addEventListener("click", () => {
    openPublicRecipesView({ scroll: true });
    history.replaceState(null, "", "#publicRecipes");
  });
}
...
if (exploreTabSets) {
  exploreTabSets.addEventListener("click", () => {
    openWeeklySetsView({ scroll: true });
    history.replaceState(null, "", "#weeklySets");
  });
}

if (exploreTabRecipes) {
  exploreTabRecipes.addEventListener("click", () => {
    openPublicRecipesView({ scroll: true });
    history.replaceState(null, "", "#publicRecipes");
  });
}

if (exploreTabSetsFromPublic) {
  exploreTabSetsFromPublic.addEventListener("click", () => {
    openWeeklySetsView({ scroll: true });
    history.replaceState(null, "", "#weeklySets");
  });
}

if (exploreTabRecipesFromPublic) {
  exploreTabRecipesFromPublic.addEventListener("click", () => {
    openPublicRecipesView({ scroll: true });
    history.replaceState(null, "", "#publicRecipes");
  });
}
...
function updateNavigation(currentView) {
  ...
  if (currentView === "landing") {
    ...
    if (pageEl) {
      pageEl.classList.remove("page--with-nav");
      pageEl.classList.remove("page--with-subnav");
    }
    return;
  }

  const showSubnav = currentView === "weeklySets" || currentView === "setBuilder";
  if (pageEl) {
    pageEl.classList.toggle("page--with-subnav", showSubnav);
  }
  ...
  if (mobileNav) {
    ...
    navItems.forEach((item) => {
      ...
      const isActive = (currentView === "app" && navType === "kondate") ||
                       ...
                       ((currentView === "publicRecipes" || currentView === "weeklySets" || currentView === "setBuilder") && navType === "publicRecipes");
      if (isActive) {
...
function handleNavClick(navType) {
  ...
  const targetHash = navHashMap[navType];
  if (targetHash) {
...
function getCurrentView() {
  ...
  if (setBuilderSection && !setBuilderSection.hidden) return "setBuilder";
  return "landing";
}
...
const PUBLIC_RECIPE_MEDIA_THEMES = [
  { name: "sunset", icon: "ramen_dining" },
  { name: "citrus", icon: "restaurant" },
  { name: "herb", icon: "soup_kitchen" },
  { name: "noodle", icon: "lunch_dining" },
];

function getPublicRecipeMediaTheme(recipeId) {
  const numeric = parseInt(String(recipeId).replace(/\D/g, ""), 10);
  const index = Number.isNaN(numeric) ? 0 : numeric;
  return PUBLIC_RECIPE_MEDIA_THEMES[index % PUBLIC_RECIPE_MEDIA_THEMES.length];
}
...
function renderPublicRecipes() {
  ...
  filtered.forEach(recipe => {
    ...
    const mediaTheme = getPublicRecipeMediaTheme(recipe.id);
    ...
    card.innerHTML = `
      ...
      <div class="recipe-card__media recipe-card__media--${mediaTheme.name}" aria-hidden="true">
        <span class="material-symbols-rounded recipe-card__media-icon">${mediaTheme.icon}</span>
      </div>
    `;
...
function importPublicRecipe(publicRecipe) {
  ...
  if (existing) {
    if (!confirm(`「${publicRecipe.name}」は既にレシピに登録されています。上書きしますか？`)) {
      return;
    }
  ...
  showToast(`「${publicRecipe.name}」をレシピに追加しました`, "success");
...
const mySetNote = document.getElementById("mySetNote");
const mySetPreviewList = document.getElementById("mySetPreviewList");
const mySetPreviewEmpty = document.getElementById("mySetPreviewEmpty");
const mySetRecipeCount = document.getElementById("mySetRecipeCount");
...
const backToSetsFromBuilder = document.getElementById("backToSetsFromBuilder");
const openAppFromBuilder = document.getElementById("openAppFromBuilder");
const openRecipesFromBuilder = document.getElementById("openRecipesFromBuilder");

const setBuilderName = document.getElementById("setBuilderName");
const setBuilderDescription = document.getElementById("setBuilderDescription");
const setBuilderTags = document.getElementById("setBuilderTags");
const setBuilderSearch = document.getElementById("setBuilderSearch");
const setBuilderSelected = document.getElementById("setBuilderSelected");
const setBuilderSelectedEmpty = document.getElementById("setBuilderSelectedEmpty");
const setBuilderList = document.getElementById("setBuilderList");
const setBuilderEmpty = document.getElementById("setBuilderEmpty");
const confirmSetBuilder = document.getElementById("confirmSetBuilder");
const cancelSetBuilder = document.getElementById("cancelSetBuilder");

const recipeCompleteModal = document.getElementById("recipeCompleteModal");
const recipeCompleteModalBackdrop = document.getElementById("recipeCompleteModalBackdrop");
const recipeCompleteClose = document.getElementById("recipeCompleteClose");
const recipeCompleteMessage = document.getElementById("recipeCompleteMessage");
const recipeCompleteAddToList = document.getElementById("recipeCompleteAddToList");
const recipeCompleteCreateSet = document.getElementById("recipeCompleteCreateSet");

const setCompleteModal = document.getElementById("setCompleteModal");
const setCompleteModalBackdrop = document.getElementById("setCompleteModalBackdrop");
const setCompleteClose = document.getElementById("setCompleteClose");
const setCompleteMessage = document.getElementById("setCompleteMessage");
const setCompleteApply = document.getElementById("setCompleteApply");
const setCompleteBack = document.getElementById("setCompleteBack");
...
function openSetBuilderView({ scroll = true, preselect = [] } = {}) {
  if (!setBuilderSection) {
    return;
  }
  landingSection.hidden = true;
  ...
  publicRecipesSection.hidden = true;
  weeklySetsSection.hidden = true;
  setBuilderSection.hidden = false;

  resetSetBuilder({ preselect });
  updateNavigation("setBuilder");

  if (scroll) {
    setBuilderSection.scrollIntoView({ behavior: "smooth", block: "start" });
  }
}
...
function renderSetCard(set, container) {
  ...
  const dishNames = set.days.flatMap((day) =>
    day.dishes.map((dish) => dish.name || (dish.recipeId ? getRecipeById(dish.recipeId)?.name : ""))
  ).filter(Boolean);
  const previewNames = dishNames.slice(0, 4);
  const previewHtml = previewNames
    .map(name => `<span class="set-card__dish">${name}</span>`)
    .join("");
  const extraHtml = dishNames.length > 4
    ? `<span class="set-card__more">他${dishNames.length - 4}品</span>`
    : "";
  ...
  card.innerHTML = `
    ...
    <div class="set-card__dishes">${previewHtml}${extraHtml}</div>
    <div class="set-card__count">${dishNames.length}品</div>
  `;
...
function openSetDetailModal(set) {
  ...
  const dishListHtml = dishNames.length > 0
    ? dishNames.map(name => `
        <li class="set-detail__dish">
          <span class="material-symbols-rounded">restaurant</span>
          <span>${name}</span>
        </li>
      `).join("")
    : `<li class="set-detail__empty">まだ料理が登録されていません</li>`;
  ...
  setDetailBody.innerHTML = `
    ...
    <p class="set-detail__note">順番は自由。今日作りたいものから選べます。</p>
...
function renderMySetPreview() {
  if (!mySetPreviewList || !mySetPreviewEmpty || !mySetRecipeCount) {
    return;
  }

  const weekStart = currentWeekStart || startOfWeek(new Date());
  const entries = getWeekDishEntries(weekStart);
  const names = entries.map(({ dish }) => {
    const recipe = dish.recipeId ? getRecipeById(dish.recipeId) : null;
    return recipe ? recipe.name : dish.draftName;
  }).filter(Boolean);

  mySetPreviewList.innerHTML = "";
  if (names.length === 0) {
    mySetPreviewEmpty.hidden = false;
    mySetRecipeCount.textContent = "0品";
    return;
  }

  mySetPreviewEmpty.hidden = true;
  names.forEach((name) => {
    const item = document.createElement("span");
    item.className = "set-create__item";
    item.textContent = name;
    mySetPreviewList.appendChild(item);
  });
  mySetRecipeCount.textContent = `${names.length}品`;
}
...
function confirmSaveMySetAction() {
  ...
  const note = mySetNote ? mySetNote.value.trim() : "";
  ...
  for (let index = 0; index < 7; index += 1) {
    ...
    const dateKey = formatDate(date);
    const dayData = getDayData(dateKey);
    const dishes = dayData.dishes.filter(d => d.recipeId || d.draftName).map((d) => {
...
if (openSetBuilderFromSetsButton) {
  openSetBuilderFromSetsButton.addEventListener("click", () => {
    openSetBuilderView({ scroll: true });
    history.replaceState(null, "", "#setBuilder");
  });
}

if (backToSetsFromBuilder) {
  backToSetsFromBuilder.addEventListener("click", () => {
    openWeeklySetsView({ scroll: true });
    history.replaceState(null, "", "#weeklySets");
  });
}

if (openAppFromBuilder) {
  openAppFromBuilder.addEventListener("click", () => {
    openAppView({ scroll: true });
    history.replaceState(null, "", "#app");
  });
}

if (openRecipesFromBuilder) {
  openRecipesFromBuilder.addEventListener("click", () => {
    openRecipesView({ scroll: true });
    history.replaceState(null, "", "#recipes");
  });
}
...
if (setBuilderSearch) {
  setBuilderSearch.addEventListener("input", () => {
    renderSetBuilderList();
  });
}

if (confirmSetBuilder) {
  confirmSetBuilder.addEventListener("click", confirmSetBuilderAction);
}

if (cancelSetBuilder) {
  cancelSetBuilder.addEventListener("click", () => {
    openWeeklySetsView({ scroll: true });
    history.replaceState(null, "", "#weeklySets");
  });
}
...
if (favoriteRecipesTab) {
  favoriteRecipesTab.addEventListener("click", () => {
    setFavoriteView("recipes");
  });
}

if (favoriteSetsTab) {
  favoriteSetsTab.addEventListener("click", () => {
    setFavoriteView("sets");
  });
}

if (favoriteSetSearch) {
  favoriteSetSearch.addEventListener("input", () => {
    renderFavoriteSetsList();
  });
}

if (recipeCompleteModalBackdrop) {
  recipeCompleteModalBackdrop.addEventListener("click", closeRecipeCompleteModal);
}

if (recipeCompleteClose) {
  recipeCompleteClose.addEventListener("click", closeRecipeCompleteModal);
}

if (recipeCompleteAddToList) {
  recipeCompleteAddToList.addEventListener("click", () => {
    if (pendingCompleteRecipe) {
      addDishToPool({ recipe: pendingCompleteRecipe });
      openAppView({ scroll: true });
      history.replaceState(null, "", "#app");
    }
    closeRecipeCompleteModal();
...
if (recipeCompleteCreateSet) {
  recipeCompleteCreateSet.addEventListener("click", () => {
    if (pendingCompleteRecipe) {
      openSetBuilderView({ scroll: true, preselect: [pendingCompleteRecipe.id] });
      history.replaceState(null, "", "#setBuilder");
    }
    closeRecipeCompleteModal();
  });
...
if (setCompleteModalBackdrop) {
  setCompleteModalBackdrop.addEventListener("click", closeSetCompleteModal);
}

if (setCompleteClose) {
  setCompleteClose.addEventListener("click", closeSetCompleteModal);
}

if (setCompleteBack) {
  setCompleteBack.addEventListener("click", () => {
    closeSetCompleteModal();
    openWeeklySetsView({ scroll: true });
    history.replaceState(null, "", "#weeklySets");
  });
...
if (setCompleteApply) {
  setCompleteApply.addEventListener("click", () => {
    if (pendingCompleteSet) {
      const targetWeekStart = startOfWeek(new Date());
      applySetToWeek(pendingCompleteSet, targetWeekStart);
      closeSetCompleteModal();
      currentWeekStart = targetWeekStart;
      currentData = loadWeekData(currentWeekStart);
      openAppView({ scroll: true });
      renderWeek(currentWeekStart);
      updateAppliedSetBanner();
      showToast(`「${pendingCompleteSet.name}」を今週にセットしました`, "success");
      return;
    }
    closeSetCompleteModal();
  });
